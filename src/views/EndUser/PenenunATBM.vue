<template>
  ...
  export default {
    ...
    mounted() {
      axios
        .get(
          `http://localhost:8081/api/v1/ulospedia/client/weavers?theLoom=ATBM&pageNo=${this.pageNo}`
        )
        .then((response) => {
    ...
    methods: {
      async loadMore() {
        const morePenenuns = await axios.get(
          `http://localhost:8081/api/v1/ulospedia/client/weavers?theLoom=ATM&pageNo=${this.pageNo}`
        )
