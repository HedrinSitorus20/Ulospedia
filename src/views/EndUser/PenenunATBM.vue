<template>
    ...
    <div class="max-w-6xl mx-auto text-center mb-10 lg:mb-14 pt-10">
      <h2 class="text-2xl font-bold md:text-4xl text-neutral_100 mb-4">Penenun Alat Tenun Bukan Mesin</h2>
  ...
  export default {
    ...
    mounted() {
      axios
        .get(
          `http://localhost:8080/api/v1/ulospedia/client/weavers?theLoom=ATBM&pageNo=${this.pageNo}`
        )
        .then((response) => {
    ...
    methods: {
      async loadMore() {
        const morePenenuns = await axios.get(
          `http://localhost:8080/api/v1/ulospedia/client/weavers?theLoom=ATM&pageNo=${this.pageNo}`
        )
