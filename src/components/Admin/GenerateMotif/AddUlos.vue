  ...
  methods: {
    async submit() {
      ...
      const responseDataText = await axios.post(
        'http://localhost:8081/api/v1/generate/ulos',
        {
          name: this.name,
          ethnic: this.ethnic
        },
      ...
      console.log(`http://localhost:8081/api/v1/generate/ulos/${newUlosDataId}/image`)

      const formData = new FormData()
      formData.append('ulos-image', this.image)

      const secondResponse = await axios.post(
        `http://localhost:8081/api/v1/generate/ulos/${newUlosDataId}/image`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      this.$emit('data', {
        ...
        imageUrl: `http://localhost:8081/api/v1/generate/ulos/${newUlosDataId}/image`
      })
